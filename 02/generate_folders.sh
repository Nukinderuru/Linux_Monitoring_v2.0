    ...
    for (( i=0; i<"$FOLDER_NUMBER"; i++ ))
        do
            if ! check_space ; then
                ...
                FOLDER_SYM=0
                while [ -e "$FOLDER_NAME" ] || [[ `expr length $FOLDER_BASE` -lt 5 ]];
                do
                ...
                WHOLE_FOLDER_NAME=$CURRENT_PATH"/"$FOLDER_NAME
                if [ -d "$WHOLE_FOLDER_NAME" ]; then
                    echo "$WHOLE_FOLDER_NAME/" `date +%Y-%m-%d-%H-%M` >> $SCRIPT_PATH/logs.txt
                    for (( y=0; y<"$FILE_NUMBER"; y++ ))
                    do
                        FILE_BASE=$(generate_base $FILE_LETTERS)
                        FILE_NAME=$WHOLE_FOLDER_NAME"/$FILE_BASE.$FILE_EXTENSION"
                        FILE_SYM=0
                        ...
                        head -c "$FILE_SIZE" </dev/urandom > "$FILE_NAME" 2>/dev/null
                        echo "$FILE_NAME" `date +%Y-%m-%d-%H-%M` `ls -lh $FILE_NAME 2>/dev/null | awk '{print $5}'` >> $SCRIPT_PATH/logs.txt
                    done
                fi
            fi
        done
